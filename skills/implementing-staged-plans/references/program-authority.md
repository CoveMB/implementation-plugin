...
A newly generated manifest-v3 proposal sets `program_start_contract` to `combined-start/v1` before publication. Shared setup validation rejects unsupported values and older manifest families. The manifest digest binds this immutable choice. Existing unmarked proposals stay on the historical setup-only route; do not migrate, delete, or mark them retroactively. Combined receipts use `setup-activation-decision/v3` and retain their complete decision, original proposal checkpoint, and genesis first-brief binding through later increments. See [Combined program start](approval-checkpoints.md#combined-program-start) for presentation and consent.

Render the new summary only as conversation/command output. Keep program details in their existing machine records; retain only summary digest and renderer metadata in checkpoints and receipts. Allocate no recap/summary file or logical role. Publication returns `program_start_summary_sha256` for this route and null `setup_recap_sha256`; legacy routes keep their original recap digest and null summary digest.

A new `implementation-program-manifest/v3` proposal is complete but unapproved. Its immutable manifest owns regular non-symlink paths for the setup-activation record, source-gate decisions, approvals, action authorizations, increment grants, rollovers, block resolutions, workspace, and status. It also owns the closed setup semantics, operation envelope, source-gate definitions, and the existing increment and closure storage descriptors. The setup record is allocated but absent, every ledger begins empty, and status is the sequence-zero proposal. Existing manifest-v2 programs retain their exact closed family and readers.

Proposal validation requires complete source, program, traceability, setup-semantic, operation-envelope, and gate-definition bindings; empty authority ledgers; an absent setup record; the unapproved workspace proposal; and sequence-zero `awaiting-program-approval` / `not-started` status. A reserved future identifier is not approval. Approved validation requires the exact setup record and its derived v2 approval receipts. Unknown validation modes and mixed schema families fail closed.
