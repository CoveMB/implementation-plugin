...
### Review allocation recovery

After acceptance, a supplemented v1 increment uses `implementation-increment-rollover/v3` to retain the original allocation binding, plan digest, baseline binding, and prior grant. The successor clears active allocation and episode bindings. Historical validation uses the same supplement owner and checks retained report bytes against the accepted delta; each later exact plan must Preserve those reports. Unsupplemented rollover records keep their existing families. Older readers reject the new rollover family.

A narrowly typed exception may authorize fresh raw review reports without replacing the original plan or baseline. It requires a setup/manifest v3 program, execution baseline v1, an unsupplemented block from `implementing`, and no persisted review, remediation, or diff-acceptance prefix. Ordinary v1 resolution never grants report allocation.

Submit `implementation-block-resolution-candidate/v2` through the existing blocked-recovery `render` command. It retains the v1 candidate's block ID and exact evidence inventory. Each ordered criterion result additionally requires a nonempty `evidence` explanation and `satisfied: true`. Assess each criterion truthfully against concrete bound evidence. Do not create this candidate when an original criterion requires completed reviews before any resume: stop for a separate decision. The generated supplement explicitly lists review completion and fresh final verification as pending work; the allocator performs neither.

The writer derives one fresh JSON report per actual selected scope beneath `recovery-<block_id>` in the original report directory. Paths are never caller-selected. The supported envelope must have one unambiguous program-owned Create class with absent-file facts and no exclusions, plus exact non-executable regular-file Preserve coverage without exclusions for every later increment. Unresolved textual exclusions or missing coverage stop; this mechanism does not interpret or expand them. The exact displayed v2 command binds scopes, paths, original plan/baseline/context/status/grant/workspace identities, original report hashes, architecture risk evidence, product fingerprint, and pending work. Direct submission grants only `allocate-review-reports` and the recorded resume, both explicit local actions.

All candidates and existing prefixes validate before the first append. Persistence remains action, resolution/supplement, then status CAS. The transaction writes no product or report file. Only byte-identical canonical prefixes are adopted; a different family, stale prompt, occupied path, changed evidence, or divergent ledger stops without overwrite. Completed retries recheck the originating episode's evidence and cannot replay into a later transition.

`review_allocation_binding` is increment-bound authority and points to the immutable originating resolution, complete original blocked context, supplement, and action. `validated_review_allocation_supplement` owns its nonrecursive provenance checks; `effective_execution_baseline` adds only exact Create report paths and absent path baselines. The persisted baseline and plan retain their original identities. Old report bytes remain immutable. The allocation-time product fingerprint does not freeze later normally authorized repairs.

A later block validates the effective contract and retains this binding in `implementation-blocked-context/v2`; its episode-specific resolution binding is still cleared. Ordinary resolution then uses the v2 command/record/binding family to bind the existing supplement and restore that episode's exact prior state. It grants no new allocation. Initial v1 contexts, records, prompts, and ordinary recovery retain their format and behavior. Unsupported families fail closed; downgrading does not remove adopted authority or constitute recovery.
