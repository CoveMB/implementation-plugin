...
For a proposal marked `combined-start/v1`, follow [Combined program start](references/approval-checkpoints.md#combined-program-start). Print the current summary and offer **Start the first increment**. In the same task, accept only a later direct reply to that presentation. In a fresh task, rediscover, revalidate, and print the summary before accepting a new direct reply. The opening handoff, creation request, an earlier answer, a quoted phrase, and generic “yes” or “proceed” cannot supply this decision. Pass the actual presented checkpoint to `adapt_program_start_decision`, then call `start_program` with fresh repository evidence.

Combined start preserves separate setup/program/workspace receipts, sequence-one `active` / `awaiting-first-increment`, and the derived first-start grant before sequence-two `active` / `preparing`. Continue through existing exact-plan and execution routing under the selected approval mode. Ask no second routine setup/start question; pause for a required source gate, standard-mode exact-plan approval, or another existing boundary. A user stop ends execution. Discovery never resumes writes: an explicit retry may recover only the original durable combined decision and its exact prefix.

An unmarked manifest-v3 proposal retains its historical readable setup recap and direct unconditional setup answer. Its setup-only activation stops at `awaiting-first-increment` without a grant; return the semantic handoff for direct submission in a fresh task. That old answer cannot become combined start consent. Preserve the legacy renderer, checkpoint, receipts, and first-start handoff bytes.
