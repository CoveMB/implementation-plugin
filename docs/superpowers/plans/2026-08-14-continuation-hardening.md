...
- `scripts/repository_preparation.py` — extend Plan A execution-baseline parsing and workspace validation to accept nonempty `inherited_paths` only for a status-current successor whose canonical rollover chain and accepted product bytes validate. Fail closed on malformed or duplicate inherited entries, missing or duplicate path-baseline matches, `Create` ownership, and user-work overlap. Do not change the schema or first-increment behavior.
...
- `tests/test_program_activation.py` — preserve Plan A public-signature and first-increment baseline assertions; add successor candidate coverage proving only validated rollover history populates `inherited_paths`, and prove invalid inherited cross-field relationships stop before materialization writes.
- `tests/test_repository_preparation.py` — preserve Plan A baseline tests and add inherited accepted-product validation, including omitted or duplicate path-baseline matches, malformed or duplicate inherited entries, `Create` ownership, and user-work overlap.
...
Validate the unique canonical rollover chain from genesis to the status-current successor. Load the predecessor's accepted product inventory and require every current product byte to match before baseline construction. Populate only the existing `inherited_paths` execution-baseline field with that normalized inventory. Extend `execution_baseline_from_value()` to require every inherited entry to be a valid normalized path with exactly one matching `path_baselines` record, reject duplicate inherited entries, require each inherited path to be classified under `Modify` or `Preserve` rather than `Create`, and keep inherited paths disjoint from `user_work_baselines`; reject every violation before materialization. Unrelated selected user work retains its original ownership. Extend the internal Plan A candidate builder only as required for that derived field, call the public exact-plan preparation/materialization interfaces without a successor-specific bypass, and preserve their signatures. For a first increment, continue emitting the exact frozen baseline bytes with `inherited_paths: []`; introduce no schema migration or alternate field.
